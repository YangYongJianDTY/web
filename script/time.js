...
var getElTop = function(el){el = el || window.event; return el.offsetTop+170; };
...
/*data.sort(function(a,b){
    a.date >b.date ? console.log("1") : console.log("2") ;
...
var tpl_content_month_show = g('tpl_content_month_show').innerHTML.replace(/^\s*/,'').replace(/\s*$/,'');
    ...
    for (month in list[year]) {
        ...
        for (var i = list[year][month].length - 1; i >= 0; i--) {
            ...
            content_month.unshift(
                ( (i==0) ?  tpl_content_month_show.replace(/\{year\}/g,year).replace(/\{month\}/g, month) : '' ) + tpl_content_month.replace(/\{year\}/g,year).replace(/\{month\}/g,month)
                    ...
                    .replace(/\{leftOrRight\}/g, i%2==0 ? 'left' : 'right')
...
//  写入内容
g('scrubber').innerHTML = '<a href="javascript:;" onclick="scrollTopTo(0)">NOW</a>'+html_scrubber.join('')+'<a href="javascript:;" onclick="scrollTopTo(getBodyH())">初见</a>';
g('content').innerHTML = html_content.join('')+ tpl_content_year.replace(/\{year\}/g,'初见').replace(/\{list\}/g,'')+'<div class="clear c_month" id="c_month_0_0"></div>'  ;
...
var showMonth = function( year , month ,elem ){
    var top = getElTop( document.getElementsByClassName('content_date_'+year+'_'+month)[0] );
    //console.log(top);
...
window.onscroll = function(){
    ...
    if( top > 200){
        ...
        g('scrubber').style.left = (getBodyW()-940) > 20 ? (getBodyW()-940)/2 : 10 + 'px';
